...
/** Soft cap for the truncated body in a pinned preview row. The
 *  pinned strip is meant to be glanceable — one line per thread —
 *  so we lop the body at ~90 chars and let the click-to-scroll
 *  affordance fetch the full content from the chronological stream
 *  below. Mirrors the reply-quote excerpt cap on ``CommentThread``
 *  so a flagged thread reads identically wherever it's previewed. */
const PINNED_PREVIEW_CHARS = 90;


function pinnedExcerpt(body: string): string {
...
/**
 * Compact preview row for the pinned strip.
 *
 * Renders author + single-line truncated body + a small reply
 * counter. The full thread (including the un-truncated root and
 * every reply) is still rendered in the chronological stream below;
 * clicking this preview scrolls the user there rather than
 * duplicating the content here. Keeps the bubble's vertical budget
 * predictable even when a flagged comment runs long.
 */
function PinnedThreadPreview({
  root,
  replyCount,
  onSelect,
}: {
  ...
  const body = root.is_deleted ? "" : pinnedExcerpt(root.body);
  return (
    <button
      type="button"
      onClick={onSelect}
