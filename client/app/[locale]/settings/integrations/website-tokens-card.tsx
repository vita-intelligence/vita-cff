...
/**
 * Website-facing access tokens card.
 *
 * The marketing site (``web-site``) presents one of these tokens as
 * ``Authorization: Bearer <raw>`` when calling NPD's
 * ``/api/website/rtg-catalog/`` (published RTG catalogue read).
 *
 * Raw token appears exactly once on mint; every list read only ever
 * shows the prefix. Copy it into the marketing site's
 * ``NPD_WEBSITE_TOKEN`` env var, then restart / redeploy the site.
 *
 * Owner-only.
 */
...
import {
  useMintWebsiteAccessToken,
  useRevokeWebsiteAccessToken,
  useWebsiteAccessTokens,
  type WebsiteAccessTokenDto,
...
export function WebsiteTokensCard({
  organization,
}: {
  organization: OrganizationDto;
}) {
  const orgId = organization.id;
  const tokensQuery = useWebsiteAccessTokens(orgId);
  const mintMutation = useMintWebsiteAccessToken(orgId);
  const revokeMutation = useRevokeWebsiteAccessToken(orgId);
  ...
  const [revokeConfirm, setRevokeConfirm] =
    useState<WebsiteAccessTokenDto | null>(null);
  ...
  const onCopy = async (raw: string) => {
    try {
      ...
      // Leave the raw visible so the operator can select + copy manually.
    }
  };

  const onRevoke = async (row: WebsiteAccessTokenDto) => {
    setBanner(null);
    try {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <h2 className="flex items-center gap-2 text-base font-semibold text-ink-1000">
            ...
            Website-facing tokens
          </h2>
          <p className="text-xs text-ink-600">
            Bearer tokens the marketing site (
            <code className="rounded bg-ink-50 px-1 py-0.5 text-[10px]">
              /products/ready-to-go
            </code>
            ) presents when reading the published RTG catalogue from
            NPD. Paste the raw token into the site&rsquo;s{" "}
            <code className="rounded bg-ink-50 px-1 py-0.5 text-[10px]">
              NPD_WEBSITE_TOKEN
            </code>{" "}
            env var. Shown once on create; only the prefix is stored
            afterwards.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5">
        {tokensQuery.isLoading ? (
          ...
          <p className="rounded-xl bg-ink-50 px-3 py-3 text-xs text-ink-600 ring-1 ring-inset ring-ink-100">
            No tokens yet. Create one above, then paste it into the
            marketing site&rsquo;s <code>NPD_WEBSITE_TOKEN</code> env
            var.
          </p>
        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-lg rounded-2xl bg-ink-0 p-5 shadow-2xl ring-1 ring-inset ring-ink-200">
            ...
            <p className="mt-1 text-xs text-ink-600">
              ...
              it into the marketing site&rsquo;s{" "}
              <code className="rounded bg-ink-50 px-1 py-0.5 text-[10px]">
                NPD_WEBSITE_TOKEN
              </code>{" "}
              env var, redeploy, then close this dialog. If you lose
              it, revoke this token here and mint a fresh one.
            </p>
      ...
      {revokeConfirm ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-ink-0 p-5 shadow-2xl ring-1 ring-inset ring-ink-200">
            ...
            <p className="mt-1 text-xs text-ink-600">
              The marketing site won&rsquo;t be able to fetch the RTG
              catalogue with this token any more. Revocation is
              immediate and can&rsquo;t be undone &mdash; mint a new
              token if you need one afterwards.
            </p>
