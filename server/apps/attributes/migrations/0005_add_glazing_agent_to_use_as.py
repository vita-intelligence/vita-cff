"""Extend the ``use_as`` controlled vocabulary with ``Glazing Agent``.

Two-step, idempotent migration:

1. Normalise every existing ``Item.attributes.use_as`` through the
   canonical map — catches any historical spellings (``Wax``,
   ``Glazing``, ``Glaze``) and folds them onto ``Glazing Agent``.
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("attributes", "0004_merge_flavour_colour_use_as"),
